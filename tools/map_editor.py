...
class EditorState:
    ...
    placements: dict[tuple[int, int, int], MapTile] = field(default_factory=dict)
...
def save_map(path: Path, placements: dict[tuple[int, int, int], MapTile]) -> None:
    ...
    for level, x, z in sorted(placements):
        tile = placements[(level, x, z)]
        lines.append("[[tiles]]")
...
def load_map(path: Path) -> dict[tuple[int, int, int], MapTile]:
    placements: dict[tuple[int, int, int], MapTile] = {}
    ...
    def commit() -> None:
        ...
        if (
            ...
            placements[(level, x, z)] = MapTile(tile_name, atlas_name)
        current = None
...
class MapEditor(tk.Tk):
    ...
    def tile_sprite_image(self, tile: TileDef, zoom: int = 1) -> tk.PhotoImage:
        if (
            zoom == 1
            and self.current_tile_image is not None
            and tile == self.current_tile
        ):
        ...
        if zoom > 1:
            return cropped.zoom(zoom, zoom)
        if tile == self.current_tile:
            self.current_tile_image = cropped
        return cropped
    ...
    def redraw_map(self) -> None:
        ...
        placements = [
            (x, z, tile)
            for (tile_level, x, z), tile in self.state_data.placements.items()
            if tile_level == level
        ]
        for x, z, saved_tile in sorted(
            placements, key=lambda item: (item[0] + item[1], item[1])
        ):
            self.draw_placement(x, z, saved_tile)
    ...
    def draw_placement(self, x: int, z: int, saved_tile: MapTile) -> None:
        self.draw_cell_outline(x, z, "#4f8cff", width=2)
    ...
    def on_map_motion(self, event: tk.Event) -> None:
        ...
        self.status.set(
            f"Level {self.current_level()} cell x={x} z={z}; left-click place, right-click erase"
        )
    ...
    def on_place_tile(self, event: tk.Event) -> None:
        if self.current_tile is None:
            ...
            return
        x, z = self.screen_to_cell(event.x, event.y)
        self.state_data.placements[(self.current_level(), x, z)] = MapTile(
            self.current_tile.name, self.current_tile.atlas_path.name
        )
        self.redraw_map()

    def on_erase_tile(self, event: tk.Event) -> None:
        x, z = self.screen_to_cell(event.x, event.y)
        self.state_data.placements.pop((self.current_level(), x, z), None)
        self.redraw_map()
    ...
    def refresh_placed_list(self) -> None:
        ...
        for key in sorted(self.state_data.placements):
            ...
            self.placed_list.insert(tk.END, f"x={x:>3} z={z:>3}  {tile.tile_name}")

    def selected_placement_key(self) -> tuple[int, int, int] | None:
